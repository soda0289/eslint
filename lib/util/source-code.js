...
function SourceCode(text, ast) {
    ...
    // Shebangs are parsed as standard line comments.
    const shebangFound = this.text.match(/^#!([^\r\n]+)/);
    const hasShebang = shebangFound && ast.comments.length && ast.comments[0].value === shebangFound[1];
