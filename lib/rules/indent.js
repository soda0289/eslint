...
class TokenInfo {
    ...
    */
     ...
     * @returns {Object} The node's indent. Contains keys `spaces` and `tabs`, representing the indent of each character.
     */
    getTokenIndent(token) {
        const indentChars = Array.from(this.sourceCode.text.slice(token.range[0] - token.loc.start.column, token.range[0]));

        return {
            space: indentChars.filter(char => char === " ").length,
            tab: indentChars.filter(char => char === "\t").length
        };
    }
}
    ...
    getDesiredIndent(token) {
        if (!this.desiredIndentCache.has(token)) {
            ...
            if (this.ignoredTokens.has(token)) {
                ...
                this.desiredIndentCache.set(token, this.tokenInfo.getTokenIndent(token)[this.indentType] / this.indentSize);
...
module.exports = {
    ...
    create(context) {
        ...
        function report(token, neededIndentLevel) {
            const actualIndent = tokenInfo.getTokenIndent(token);
        ...
        function validateTokenIndent(token, desiredIndentLevel) {
            const tokenIndent = tokenInfo.getTokenIndent(token);
            ...
            const goodChars = indentType === "space" ? tokenIndent.space : tokenIndent.tab;
            const badChars = indentType === "space" ? tokenIndent.tab : tokenIndent.space;

            return goodChars === desiredIndentLevel * indentSize && badChars === 0;
        }
        ...
        */
        ...
        function addBlockIndent(node) {
            ...
            offsets.setDesiredOffsets(tokens.slice(1, -1), tokens[0], blockIndentLevel);
        ...
        */
        function addElementListIndent(tokens, elements, offset) {
            ...
            */
            ...
            offsets.setDesiredOffsets(tokens.slice(1, -1), tokens[0], offset === "first" ? 1 : offset);
        ...
        */
        function addFunctionParamsIndent(node, paramsIndent) {
            ...
            if (typeof paramsIndent !== "number" && typeof paramsIndent !== "string") {
                node.params.forEach(param => offsets.ignoreToken(sourceCode.getFirstToken(param)));
            }
        }
        ...
        */
        function addBinaryOrLogicalExpressionIndent(node) {
            ...
            offsets.setDesiredOffsets(tokensAfterOperator.slice(1), tokensAfterOperator[0], 1);
        }
        ...
        */
        function addFunctionCallIndent(node) {
            ...
            if (typeof options.CallExpression.arguments === "number" || typeof options.CallExpression.arguments === "string") {
                ...
                node.arguments.forEach(arg => offsets.ignoreToken(sourceCode.getFirstToken(arg)));
            }
        }
        ...
        */
        function addClassIndent(node) {
            ...
            offsets.setDesiredOffsets(tokens.slice(1, -1), tokens[0], 1);
            offsets.matchIndentOf(tokens[0], tokens[tokens.length - 1]);
        }
        ...
        return {
            ...
            ConditionalExpression(node) {
                ...
                if (!(node.parent.type === "ConditionalExpression" && options.flatTernaryExpressions)) {
                    offsets.setDesiredOffsets(tokens.slice(1), tokens[0], 1);
                }
            },
            ...
            MemberExpression(node) {
                ...
                offsets.setDesiredOffsets(tokensToIndent.slice(1), tokensToIndent[0], 0);
            ...
            VariableDeclaration(node) {
                offsets.setDesiredOffsets(getTokensAndComments(node).slice(1), sourceCode.getFirstToken(node), options.VariableDeclarator[node.kind]);
            ...
            "Program:exit"() {
                ...
                sourceCode.lines.forEach((line, lineIndex) => {
                    ...
                    if (!tokenInfo.firstTokensByLineNumber.has(lineNumber)) {
                        ...
                        // Don't check indentation on black lines
                        return;
                    }
