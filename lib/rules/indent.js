...
module.exports = {
        ...
        schema: [
            ...
            },
            {
                ...
                properties: {
                    ...
                    ArrayExpression: ELEMENT_LIST,
                    ObjectExpression: ELEMENT_LIST,
    ...
    create(context) {
        ...
        const lockedFirstTokens = new WeakMap();
        ...
        */
        function isFirstTokenOfLine(token) {
            return getFirstTokenOfLine(token) === token;
        }
        ...
        */
        function getDesiredIndent(token) {
            if (!desiredIndentCache.has(token)) {
                ...
                if (ignoredTokens.has(token)) {
                ...
                } else if (lockedFirstTokens.has(token)) {
                     ...
                     */
                    const firstToken = lockedFirstTokens.get(token);

                    desiredIndentCache.set(token,

                        // (indentation for the first element's line)
                        getDesiredIndent(getFirstTokenOfLine(firstToken)) +

                            // (space between the start of the first element's line and the first element)
                            (firstToken.loc.start.column - getFirstTokenOfLine(firstToken).loc.start.column) / indentSize
                    );
                } else {
        ...
        */
        ...
        function addBlockIndent(node) {
             ...
             * }
             */
            const tokenToMatchAgainst = isFirstTokenOfLine(tokens[0]) ? tokens[0] : sourceCode.getFirstToken(node.parent);
        ...
        function addElementListIndent(tokens, elements, offset) {
            ...
            */
            ...
            elements.forEach((element, index) => {
                ...
                if (offset === "first" && isFirstTokenOfLine(getFirstToken(element))) {
                    lockedFirstTokens.set(getFirstToken(element), getFirstToken(elements[0]));
                } else {
        ...
        */
        ...
        return {
            ...
            MemberExpression(node) {
                ...
                if (node.computed) {
                    matchIndentOf(firstNonObjectToken, sourceCode.getLastToken(node));
                }
                ...
                if (typeof options.MemberExpression !== "number") {
                    tokensToIndent.forEach(token => {
                        matchIndentOf(getFirstTokenOfLine(token), token);
                        ignoreToken(token);
                    });
                } else if (node.object.loc.end.line === node.property.loc.start.line) {
                    setDesiredOffsets(tokensToIndent, getFirstTokenOfLine(sourceCode.getLastToken(node.object)), options.MemberExpression);
                } else {
            ...
            "Program:exit"() {
                ...
                sourceCode.ast.tokens.forEach(token => {
                    ...
                    // If a token doesn't match its expected indentation, report it.
                    if (isFirstTokenOfLine(token) && !validateTokenIndent(token, tokenIndent)) {
                ...
                sourceCode.ast.comments.forEach(comment => {
                    ...
                    // If the comment is not at the start of its line, or it matches its own expected indentation, don't report it.
                    if (!isFirstTokenOfLine(comment) || validateTokenIndent(comment, getDesiredIndent(comment))) {
                        return;
                    }

                    const tokenBefore = precedingTokens.get(comment);
                    const tokenAfter = tokenBefore ? sourceCode.getTokenAfter(tokenBefore) : sourceCode.ast.tokens[0];

                    // If the comment matches the expected indentation of the token immediately before or after, don't report it.
                    if (
                        (tokenBefore && validateTokenIndent(comment, getDesiredIndent(tokenBefore))) ||
                        (tokenAfter && validateTokenIndent(comment, getDesiredIndent(tokenAfter)))
                    ) {
                        return;
                    }

                    // Otherwise, report the comment.
